...
public function exportarArchivos()
    {
        ...
        $query = "SELECT ID_reloj_Bi, ID_cedula_Bi, nombre_empleado FROM Planilla_Bisemanal";
    ...
    if ($resultado->num_rows > 0) {
        while ($fila = $resultado->fetch_assoc()) {
            $sheet->setCellValue('A' . $row, $fila['ID_reloj_Bi']);
            $sheet->setCellValue('B' . $row, $fila['ID_cedula_Bi']);
